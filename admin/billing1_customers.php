...
<div class="cloth">
	...
	<div class="shirt">
		...
		<div class="shirt_right">
			...
			<form method="get" action="billing1_customers.php" name="form2" onSubmit="return checkvalue2()">
			<table width="100%">
				<tr>
					
					<td width="26%">
						ชื่อลูกค้า : 
					  <input name="button" type="button" value="กลับ" onclick="window.location='billing1.php';"/>
                      <? if($_GET[fix]=="true"){?>
					  <input type="hidden" name="fix" id="fix"  value="true"/>
					  <input type="hidden" name="billing_id" id="billing_id" value="<?=$_GET[billing_id]?>" />
...
<!--  <option value="customers_name" <?php if (!(strcmp($_GET[search1], "customers_name"))) {echo "selected=\"selected\"";} ?>>ชื่อ</option>
 --></select>			    </td>
	  ...
	  <td width="11%">
			  ...
			  <input type="button" name="Submit"   value="ทั้งหมด" onClick="window.location='billing1_customers.php?all=true<?=$edit?>';"/>
				  </td>
			  </tr>
			</table>
			</form>
...
<br/>
		...
		<td>
			<a href="billing1_customers_p.php?<? if($_GET[fix]=="true"){echo "fix=true&billing1_id=$_GET[billing1_id]&edit=true";}else{echo"action=add";}?>&customers_id=<?=$rs["customers_id"];?>">
