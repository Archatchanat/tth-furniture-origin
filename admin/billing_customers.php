		...
		<div class="shirt_right">
			...
			<table width="100%">
				<tr>
					...
					<td width="26%">
						ชื่อลูกค้า : 
					  <input name="button" type="button" value="กลับ" onclick="window.location='<? if($_GET[fix]=="true"){?>billing.php?fix=true&orderid_id=<?=$_GET[orderid_id];?>&orderid_num=<?=$_GET[orderid_num];?><? }else{?>billing.php<? }?>';"/>
                      <? if($_GET[fix]=="true"){?>
					  <input type="hidden" name="fix" id="fix"  value="true"/>
					  <input type="hidden" name="orderid_id" id="orderid_id" value="<?=$_GET[orderid_id]?>" />
                       <input type="hidden" name="orderid_num" id="orderid_num" value="<?=$_GET[orderid_num]?>" />
...
<!--  <option value="customers_name" <?php if (!(strcmp($_GET[search1], "customers_name"))) {echo "selected=\"selected\"";} ?>>ชื่อ</option>
 --></select>			    </td>
	  ...
	  <td width="11%">
			  ...
			  <input type="button" name="Submit"   value="ทั้งหมด" onClick="window.location='billing_customers.php?all=true<?=$edit?>';"/>
				  </td>
			  </tr>
			</table>
			</form>
...
<br/>
		...
		<td>
			<a href="billing_customers_p.php?<? if($_GET[fix]=="true"){echo "fix=true&orderid_id=$_GET[orderid_id]&edit=true&orderid_num=$_GET[orderid_num]";}else{echo"action=add";}?>&customers_id=<?=$rs["customers_id"];?>">
