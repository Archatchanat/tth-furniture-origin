...
<div class="cloth">
		...
		<div class="shirt_right">
			...
			while($rs_one = mysql_fetch_array($get_one))
			{
				...
				echo $customers_com    = $rs_one['customers_com'];
					  ...
					  <input name="button" type="button" value="กลับ" onclick="window.location='<? if($_GET[fix]=="true"){?>billing_customers.php?fix=true&orderid_id=<?=$_GET[orderid_id]?>&edit=true&customers_id=<?=$_GET[customers_id]?>&orderid_num=<?=$_GET[orderid_num];?><? }else{?>billing_customers.php<? } ?>';"/>
                      <? if($_GET[fix]=="true"){?>
	...
	?>
  	<tr id="tr<?=$i?>" onMouseOver='mOvr(this,"#E6E6E6");' onMouseOut='mOut(this,"#ffffff");'>
		...
		<td>
			<a href="billing.php?<? if($_GET[fix]=="true"){echo "fix=true&orderid_id=$_GET[orderid_id]&edit=true&customers_p_id=$rs[customers_p_id]&orderid_num=$_GET[orderid_num]";}else{echo"action=add";}?>&customers_id=<?=$rs["customers_id"];?>&customers_p_id=<?=$rs["customers_p_id"];?>">
