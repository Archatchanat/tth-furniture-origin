...
<div class="cloth">
		...
		<div class="shirt_right">
			...
			while($rs_one = mysql_fetch_array($get_one))
			{
				...
				echo $customers_com    = $rs_one['customers_com'];
					  ...
					  <input name="button" type="button" value="กลับ" onclick="window.location='<? if($_GET[copy]=="true"){?>billing_customers_copy.php?copy=true&orderid_id=<?=$_GET[orderid_id]?>&edit=true&customers_id=<?=$_GET[customers_id]?>&orderid_num=<?=$_GET[orderid_num];?><? }else{?>billing_customers.php<? } ?>';"/>
                      <? if($_GET[copy]=="true"){?>
		...
		<tr>
			<td width="56%">
				<? if($_GET['type_stack']=="" || $_GET['type_stack']=="DESC"){?>
   					<a href="?stack=customers_p_name&amp;type_stack=ASC" style="text-decoration:none; color:#000000;">ชื่อโปรเจค</a><a href="?stack=customers_p_name&type_stack=ASC" style="text-decoration:none; color:#000000;"></a>
				<? }else { ?>
   					<a href="?stack=customers_p_name&amp;type_stack=ASC" style="text-decoration:none; color:#000000;">ชื่อโปรเจค</a><a href="?stack=customers_p_name&type_stack=DESC" style="text-decoration:none; color:#000000;"></a>
				<? } ?>			</td>
			<td width="32%"><a href="?stack=orderid_date1&amp;type_stack=ASC" style="text-decoration:none; color:#000000;">หมวดหมู่โปรเจค</a> </td>
	...
	?>
  	<tr id="tr<?=$i?>" onMouseOver='mOvr(this,"#E6E6E6");' onMouseOut='mOut(this,"#ffffff");'>
		...
		<td>
			<a href="billing.php?<? if($_GET[copy]=="true"){echo "copy=true&orderid_id=$_GET[orderid_id]&edit=true&customers_p_id=$rs[customers_p_id]&orderid_num=$_GET[orderid_num]";}else{echo"action=add";}?>&customers_id=<?=$rs["customers_id"];?>&customers_p_id=<?=$rs["customers_p_id"];?>">
		...
		<tr>
		  ...
		  <td width="70%" align="right">
				...
				if($Prev_Page)
				{
					...
					echo " <a href='$_SERVER[SCRIPT_NAME]?Page=$Prev_Page&stack=".$_GET['stack']."&type_stack=".$_GET['type_stack']."$edit'><< Back</a> ";
				}

				for($i=1; $i<=$Num_Pages; $i++){
					if($i != $Page)
					{
						echo "<a href='$_SERVER[SCRIPT_NAME]?Page=$i&stack=".$_GET['stack']."&type_stack=".$_GET['type_stack']."$edit'> $i </a>";
					}
					else
					{
				...
				if($Page!=$Num_Pages)
				{
					echo " <a href ='$_SERVER[SCRIPT_NAME]?Page=$Next_Page&stack=".$_GET['stack']."&type_stack=".$_GET['type_stack']."$edit'>Next>></a> ";
				}
				?>
				</div>	
			</td>
