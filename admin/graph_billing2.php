...
if($_GET['all']=="true"){
		$_SESSION["str_FromYY2"] = "";
	}
	if($_GET['FromYY2']!=""){
		$_SESSION["str_FromYY2"] = $_GET['FromYY2'];
	}
	...
	<div class="shirt">
		...
		<div class="shirt_right">
			...
			<form method="get" action="#" name="form2" onSubmit="return checkvalue2()">
			  <table width="100%">
                <tr>
                  <td width="34%"> กราฟสรุปยอดใบวางบิล :                 </td>
                  <td width="47%" align="right">ค้นหาจากปี::
                    <select name="FromYY2" id="FromYY2" class="txtbox">
<? $tel_one = "SELECT billing_date FROM billing group by Year(billing_date)	";
			...
			{ ?>
  <option value="<?=Yearform1($rs_one[billing_date])?>">
<?=Yearform1($rs_one[billing_date])?>
 ...
 <? } ?>
                  ...
                  <td width="10%"> |
                    <input type="button" name="Submit2"   value="ทั้งหมด" onclick="window.location='graph_billing2.php?all=true';"/>
                  </td>
                </tr>
              </table>
		  </form>
...
$strCurrDate = strtotime($inputDate11);
 $days=date("Y-m-d", mktime(date("H",$strCurrDate)+0, date("i",$strCurrDate)+0, date("s",$strCurrDate)+0, date("m",$strCurrDate)+1  , date("d",$strCurrDate)+0, date("Y",$strCurrDate)+0));
		...
		$tel_one = "select * 
			...
			$tel_one1 = "select   SUM(billing.billing_amount_sum) as  billing_amount_sum	
						from    billing , customers_project
						where  billing.customers_p_id = customers_project.customers_p_id and customers_project.project_c_id='$rs_one[project_c_id]'
          ...
          [' <? echo "$rs_one[project_c_name]";?>',<? if ($rs_one1[billing_amount_sum]==""){echo "0";}else{echo "$rs_one1[billing_amount_sum]";}?>],
	...
	?>
      <table width="100%" border="0" cellspacing="0">
        <?
		if ($_SESSION["str_FromYY2"]!=""){
		// query area
		  $inputDate = '01-01'.'-'."$_SESSION[str_FromYY2]";
	...
	?>
          <tr id="tr<?=$i?>" onmouseover='mOvr(this,&quot;#E6E6E6&quot;);' onmouseout='mOut(this,&quot;#ffffff&quot;);'>
            ...
            <td><?=monthform1($rs['billing_date'])?>
                <br/></td>
            <td><a href="graph_billing2.php?view=true&amp;billing_date=<?=monthform2($rs['billing_date'])?>"> <img src="images/view.png" onmouseover="this.style.width = '20px'" onmouseout="this.style.width = '16px'"/> </a> </td>
            </tr>
          <?
		}
		  }
