...
if($_GET['all']=="true"){
	...
	if($_GET['customers_p_id']!=""){
		...
		$_SESSION["str_customers_name"] = $_GET['customers_name'];
			}
	if($_GET['customers_p_id']==""){
			...
			<form method="get" action="billing.php" name="form2" onSubmit="return checkvalue2()">
			  <table width="100%">
                <tr>
                  ...
                  <input name="button" type="button" value="กลับ" onclick="window.location='project_search.php?project_c_id=<?=$_SESSION["str_project_c_id"]?>&search=<?=$_SESSION["str_search"]?>';"/></td>
                </tr>
              </table>
		  </form>
			<hr/>
...
<form name="list" method="post" action="?action=del_sel" onsubmit="return ConfAll()">
	  <table width="100%" border="0" cellspacing="0">
		...
		$search=$_GET[customers_p_id];
			$tel = "select orderid_id,orderid_co,orderid_attn,orderid_publish,orderid_date1 from    orderid 
					LEFT JOIN customers_project ON (orderid.customers_p_id=customers_project.customers_p_id)
					where  
						   orderid.customers_p_id = '".$_SESSION["str_customers_p_id"]."' and orderid_publish1='Yes'  and orderid.customers_id='".$_GET['search']."'
					UNION
					select billing_id,billing_co,billing_attn,billing_publish,billing_date1 from    billing  
					LEFT JOIN customers_project ON (billing.customers_p_id=customers_project.customers_p_id)
					where  
						   billing.customers_p_id = '".$_SESSION["str_customers_p_id"]."'  and  billing.customers_id='".$_GET['search']."'
					UNION
					select billing2_id,billing2_co,billing2_attn,billing2_publish,billing2_date1 from    billing2  
					LEFT JOIN customers_project ON (billing2.customers_p_id=customers_project.customers_p_id)
					where  
						   billing2.customers_p_id = '".$_SESSION["str_customers_p_id"]."' and  billing2.customers_id='".$_GET['search']."'
	...
	?>
          <tr id="tr<?=$i?>" onmouseover='mOvr(this,&quot;#E6E6E6&quot;);' onmouseout='mOut(this,&quot;#ffffff&quot;);'>
            ...
            <td><a href="MPDF56/examples/<? if (!(strcmp($orderid_idts, "Qu"))) {echo"example01_basic.php?orderid_id=$rs[orderid_id]";};
